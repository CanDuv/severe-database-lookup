...
2. It performs a constant-time match against the compact profile-ID index bundled from the supplied CSV.
...
The app bundles only a compact profile-ID index, not the raw CSV. To rebuild it from a newer export:

```bash
npm run build:data -- "C:\path\to\Severe Injection Database - Refined Raw Data.csv"
```

The generated `data/cheater-user-ids.json` is the deployed lookup index. It contains only deduplicated numeric profile IDs and supports an in-memory O(1) set membership check.
